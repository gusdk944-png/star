...
호스팅 서비스)이 그 창고를 읽어서 자동으로 사이트를 열어줘요. 네이버 클로바 API 키(비밀
...
## 4단계. 네이버 클로바 스튜디오 키 발급받기

1. https://www.ncloud.com 에서 네이버 클라우드 플랫폼 계정을 만드세요 (깃허브·Vercel과는
   별개의 계정이에요)
2. 결제 수단(카드 또는 계좌이체)을 등록하세요 — 사용한 만큼만 요금이 나가요
3. 콘솔에서 **CLOVA Studio** 메뉴로 들어가세요
4. 왼쪽 메뉴의 **API 키** 화면에서 API 키를 발급받으세요
5. 발급된 키를 복사해서 안전한 곳에 잠깐 메모해두세요 (다시 안 보여줄 수 있어요).

## 5단계. Vercel에 키 등록하기

1. Vercel에서 방금 배포한 프로젝트로 들어가세요
2. **Settings → Environments → Production** 클릭 (버전에 따라 "Environment
   Variables"라는 이름으로 보일 수도 있어요)
3. Key: `NCP_CLOVASTUDIO_API_KEY` / Value: 4단계에서 복사한 API 키를 등록하세요
   (기존에 있던 `ANTHROPIC_API_KEY`는 지워도 되고 그냥 둬도 상관없어요)
...
- `api/claude.js` — 서버 역할. 네이버 클로바 API 키는 여기서만 사용되고, 손님
  브라우저에는 절대 노출되지 않아요.
